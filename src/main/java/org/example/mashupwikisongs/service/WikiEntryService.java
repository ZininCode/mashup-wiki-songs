...
public class WikiEntryService {
    ...
    public String getEntry(String mbid) {
        ...
        try {
            ...
            log.info("the relation of the artist with {mbid} for Wikipedia was not found in MusicBrainz", mbid);
        }
        ...
        try {
            ...
            while (scanner.hasNextLine()) {
                ...
                if (line.contains(wikiLinkInMusicBrainz)) {
                    ...
                    wikiEntry = URLDecoder.decode(wikiEntry, "UTF-8");
                    break;
                }
            }
            scanner.close();
        } catch (IOException e) {
            log.info("A link of the artist with {mbid} to Wikipedia was not found in MusicBrainz artist's page", mbid);
        }
        return wikiEntry;
    }
}
